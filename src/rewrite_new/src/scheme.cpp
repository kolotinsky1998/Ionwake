...
Scheme::Scheme(Converter converter_, int nx_, int ny_, int nz_, 
    ...
    hxi(hxi_),ximax(ximax_)
    {
        ...
        ftime = new double *****[nx];
        for (int i=0; i<nx; i++){
            ftime[i] = new double**** [ny];
            for (int j=0; j<ny; j++){
                ftime[i][j] = new double*** [nz];
                for (int k=0; k<nz; k++){
                    ftime[i][j][k] = new double** [nvx];
                    for (int a=0; a<nvx; a++){
                        ftime[i][j][k][a] = new double* [nvy];
                        for (int b=0; b<nvy; b++){
                            ftime[i][j][k][a][b] = new double [nvz];
                        }
                    } 
                }
            }
        ...
        for (int i=0; i<nx; i++){
            az[i] = new double *[ny];
            for (int j=0; j<ny; j++){
                az[i][j] = new double [nz];
            }
        }
        maxvell = new double **[nvx];
        for (int a=0; a<nvx; a++){
            maxvell[a] = new double *[nvy];
            for (int b=0; b<nvy; b++){
                maxvell[a][b] = new double [nvz];
            }
        }
    ...
    for (int i=0; i<nx; i++){
        for (int j=0; j<ny; j++){
            for (int k=0; k<nz; k++){
                for (int a=0; a<nvx; a++){
                    for (int b=0; b<nvy; b++){
                        delete[] ftime[i][j][k][a][b];
                    }
                    delete[] ftime[i][j][k][a];
                }
                delete[] ftime[i][j][k]; 
            }
            delete[] ftime[i][j];
                    ...
                    if(vx[a]>0){
                        for (int b=1; b<nvy-1; b++){
                            for (int c=1; c<nvz-1; c++){
                                if (i==0){
                                    f[i][j][k][a][b][c] = ftime[i][j][k][a][b][c] - vx[a]*(ftime[i][j][k][a][b][c] - ftime[nx-1][j][k][a][b][c])*dt/hx;
                                } else {
                        ...
                        for (int b=1; b<nvy-1; b++){
                            for (int c=1; c<nvz-1; c++){
                                if (i==nx-1){
                                    f[i][j][k][a][b][c] = ftime[i][j][k][a][b][c] - vx[a]*(ftime[0][j][k][a][b][c] - ftime[i][j][k][a][b][c])*dt/hx;
                                } else {
                ...
                for (int a=1; a<nvx-1; a++){
                    for (int b=1; b<nvy-1; b++){
                        if(vy[b]>0){
                            for (int c=1; c<nvz-1; c++){
                                if (j==0){
                                    ftime[i][j][k][a][b][c] = f[i][j][k][a][b][c] - vy[b]*(f[i][j][k][a][b][c] - f[i][ny-1][k][a][b][c])*dt/hy;
                                } else {
                            ...
                            for (int c=1; c<nvz-1; c++){
                                if (j==ny-1){
                                    ftime[i][j][k][a][b][c] = f[i][j][k][a][b][c] - vy[b]*(f[i][0][k][a][b][c] - f[i][j][k][a][b][c])*dt/hy;
                                } else {
                ...
                for (int a=1; a<nvx-1; a++){
                    for (int b=1; b<nvy-1; b++){
                        for (int c=1; c<nvz-1; c++){
                            if(vz[c]>0){
                                if (k==0) {
                                    f[i][j][k][a][b][c] = ftime[i][j][k][a][b][c] - vz[c]*(ftime[i][j][k][a][b][c] - ftime[i][j][nz-1][a][b][c])*dt/hz;
                                } else {
                                    f[i][j][k][a][b][c] = ftime[i][j][k][a][b][c] - vz[c]*(ftime[i][j][k][a][b][c] - ftime[i][j][k-1][a][b][c])*dt/hz;
                                }
                            } else {
                                if (k==nz-1){
                                    f[i][j][k][a][b][c] = ftime[i][j][k][a][b][c] - vz[c]*(ftime[i][j][0][a][b][c] - ftime[i][j][k][a][b][c])*dt/hz;
                                } else {
